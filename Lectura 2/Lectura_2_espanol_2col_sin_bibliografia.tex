...
La computación en GPU es sin duda un área de investigación bien establecida, desde el lanzamiento de plataformas de propósito
general como CUDA y OpenCL. En el modelo de
...
UTM y (3) organización de threads por bloque sin comprometer.
...
computacionales son de hecho td-problems: mapas de distancias euclidianas (EDM), detección de colisiones, recorrido de
grafos a través de matrices de adyacencia, Autómatas Celulares
(por ejemplo, el Juego de la Vida de John Conway) en dominios
triangulares, inversión de matrices, descomposición LU/Cholesky, entre otros.
...
ADN, que se basa en EDM. En su trabajo, los autores mencionan
...
cómputo. Li et al. también han trabajado en EDMs basados en GPU
...
Jung et al. propusieron en 2008 estructuras de datos compactas
para representar matrices triangulares y simétricas con aplicaciones a la
descomposición LU y Cholesky. La estrategia se basa en construir
...
inversión de matrices triangulares. Los autores identifican que
...
detección de colisiones basada en las propiedades del mapa triangular superior. El mapa, denominado UTM, es una función
...
(basada en el algoritmo de aproximación de Newton-Raphson) para
...
estrategia BB, donde los autores reportan un speedup de casi 2X,
...
la función de mapeo, tal como en el mapa UTM. En el mapeo por
...
Al comparar LTM con su contraparte más cercana UTM, identificamos
...
asintóticamente $O(M(n))$, donde $M(n)$ es el costo de
...
usando tres iteraciones del método de Newton-Raphson.
...
siguiendo los detalles proporcionados por los autores.
...
La estrategia de rectangular box (RB) se basa en el trabajo de Jung et al. Este método toma la porción sub-triangular de los threads donde
...
La estrategia de recursive partition (REC) fue propuesta para la GPU
 para la inversión de matrices. En este método, el tamaño del
...
en. En el trabajo original, el mapeo de los bloques a sus
...
El upper-triangular mapping (UTM) fue propuesto por Avril et al. para realizar detección de colisiones eficiente en la GPU. Este método es muy
...
upper-triangular mapping (UTM), rectangular box (RB) 
y recursive partition (REC). Verificamos que las salidas para
...
upper-triangular map (UTM). Es importante mencionar que el tamaño
